...
def project_root() -> Path:
    env = os.environ.get("TICKETDESK_ROOT", "").strip()
    ...
    for cand in candidates:
        if cand in seen:
            continue
        seen.add(cand)
        if (cand / "docs" / "policy").is_dir() and (cand / "fixtures" / "tickets").is_dir():
            return cand
        if cand.name == "ticketdesk" and (cand / "docs" / "policy").is_dir():
            return cand
    raise FileNotFoundError("找不到工单台项目根（需要 docs/policy 与 fixtures/tickets）。设置 TICKETDESK_ROOT。")
