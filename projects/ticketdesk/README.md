...
面试官要你先讲业务时，看下面「面试：业务背景」或 [docs/jobs/business.md](../../docs/jobs/business.md)。走读请跳到「从 0 到 1」。
...
完整口述、三句业务题、夹具表：[docs/jobs/business.md](../../docs/jobs/business.md)。下面这一段开场能背。简历三条 / STAR 不要删，那是取舍，不是业务开场。

**30 秒。** 我走读的第一个队列，是虚构纸墨店「青匣记」的售后工单台。顾客进门不是来闲聊的，是来处理一笔已经成交的买卖：仅退款、退货退款、换货；大促期物流延误发券，不发现金。Agent 坐在工单里，做分流、引用当天仍生效的政策、在闸门停手。退款接口形状像生产，必须人点执行；演示永远 `confirm_required`，不打款。

**2 分钟。** 青匣记卖纸墨。盛夏大促快递挤，顾客拿活动页来要现金——日常写「不赔运费」，活动写「48 小时发不超过 12 块的券」。店里的人一天对着 Inbox：核单号、对实付、看仓库有没有入库、写对客稿和对内备注。货款走优惠后实付，活动走券，换货不现金；已经退过的同一笔损失不能再补。七天无理由过了签收七日，「不想要了」不够；漏液不受这七日限制。三件套只裂砚台，只退那一行实付。我把 agent 卡在分类（缺单号不退款）、政策（活动必须点名 `promo-2026-summer.md`）、闸门（超 200 / 未入库 / 夜间二线空都停在人）。

**我拒绝把 agent 做成**客服聊天机器人，也拒绝做成「AI 自动打款」。进的是工单对象，出的是引用和闸门。
