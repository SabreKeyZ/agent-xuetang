...
第 1–5 周你手写过小脚本。第 6 周起要走读已经写好的工单台。中间缺一课：怎么把助手当学徒用，而不是把整份规格倒进去等聊天机器人。
...
**先修。** 第 3 周 `path:line`、第 5 周三个出口。第 0 周的 venv。本插页不打网。

读本页 + 开助手 30 分钟；五步提示各 20–30 分钟；验 diff 和失败对照 30 分钟。合计约 2–3 小时。卡超过 40 分钟去 [FAQ](../faq.md)「可以不手写全部代码吗」。

**对应视频：** 本插页不配新视频。口播仍用 [第 3 周](03-memory-rag.md) / [第 5 周](05-multi-agent.md) 那两行。白天先贴提示、跑评测。
...
`python -m minidesk demo` 能指着芯片或红条，只说明最小闸门在。打开 [第 6 周](06-ticketdesk.md)，走读 `projects/ticketdesk`。迷你台**还没有**：

1. **Inbox UI**（灰/白气泡、芯片可点、执行钮）
2. **部分退**（多 SKU 只退损坏行实付）
3. **七天无理由**（以及退货未入库、双重 SLA、夜间二线空）

不要以为作业做完了。第 6 周的验收仍是那一页的勾选框。
...
4. 绿了之后，合上迷你台，列出工单台比它多的三件事（不要抄本页列表当自己的话，打开第 6 周夹具名再说）。
...
希望听到：`我用助手从空目录搭过带引用和人确认闸门的工单台，并自己验过 diff`。补一句：评测是 `labs/vibe-minidesk/evals`，退款永远 `confirm_required`，假引用我拒过。不要讲「全交给 Cursor」。不要讲工单台源码是你手写的——第 6 周那份不是。
...
- 学生入口：[labs/vibe-minidesk/README.md](../../labs/vibe-minidesk/README.md)
- 第 3 周引用：[03-memory-rag.md](03-memory-rag.md)
- 第 6 周工单台走读：[06-ticketdesk.md](06-ticketdesk.md)
