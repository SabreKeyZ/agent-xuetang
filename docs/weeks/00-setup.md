...
> **本班属于 1 个月路径的第 1 周**（前半：环境，工作日 1–2 晚）。同周还有 [01](01-what-is-an-agent.md)、[02](02-tools-and-react.md)。
...
本班约 2 小时核心（晚上一晚）。同周另外两班合计约 8–10 小时，不要把三篇加成 16 小时再吓自己。Key / hello_chat 可选，不占默认工期。

| 块 | 时间 | 做什么 |
| --- | --- | --- |
| 装环境 | 1 小时 | Git、Python、venv |
| 跑演示 | 1 小时 | `pip install -e` 两台，demo + serve |
| （可选）申请 Key | 另算 | DeepSeek / 智谱 / 通义 三选一，或装 Ollama |
| （可选）hello_chat | 另算 | 改 `.env`，看返回 |
| 缓冲 | 卡了再加 | 代理、镜像、Windows 编码，见 [FAQ](../faq.md) |
